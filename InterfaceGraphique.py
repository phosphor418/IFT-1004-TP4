...
class Parametres_partie(Toplevel):
    def __init__(self, parent):
        ...
        self.box_nb_joueurs = Combobox(self, textvariable=self.var_nb_joueurs, values=[2, 3, 4])

        self.box_nb_joueurs.grid(row=0, column=1, padx=5, pady=5)

        self.instruction_nb_joueurs = Label(self, text="Inclure l'ordinateur")
        self.instruction_nb_joueurs = Label(self, text="R2D2 va jouer?")
        self.instruction_nb_joueurs.grid(row=1, column=0, padx=5, pady=5)
        self.var_ajout_ordi = StringVar()
        self.var_ajout_ordi.trace('w', self.selection_nb_joueur)
        self.box_ajout_ordi = Combobox(self, textvariable=self.var_ajout_ordi, values=["Non", "Oui"])
        self.box_ajout_ordi.grid(row=1, column=1, padx=5, pady=5)
    ...
    def valider(self):
        ...
        try:
            ...
            self.nom_joueurs = [self.list_vars_nom_joueurs[i].get()
                                for i in range(self.nombre_joueurs - self.inclure_ordi)]
            if not (2 <= self.nombre_joueurs <= 4):
    ...
    def selection_nb_joueur(self, index, value, op):
        ...
        for i in range(self.var_nb_joueurs.get() - t):
...
class InterfaceGraphique(Tk):
    ...
    def __init__(self):
        ...
        # images des dés
        self.images_des = [PhotoImage(file="ressources/dice_1.png"), PhotoImage(file="ressources/dice_2.png"),
                           PhotoImage(file="ressources/dice_3.png"), PhotoImage(file="ressources/dice_4.png"),
                           PhotoImage(file="ressources/dice_5.png"), PhotoImage(file="ressources/dice_6.png")]
